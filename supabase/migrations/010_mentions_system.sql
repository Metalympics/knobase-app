...
-- - users table (exists)
-- - pages table (editor content — not the KB documents table)
-- - agents table (exists)
...
ALTER TABLE public.notifications 
ADD COLUMN IF NOT EXISTS document_id UUID REFERENCES public.pages(id) ON DELETE CASCADE;
...
-- ---------------------------------------------------------------------------
-- Auto-notification trigger function
-- ---------------------------------------------------------------------------
-- Called AFTER INSERT on public.mentions via trg_create_notification_on_mention.
--
-- Behavior:
--   1. When a new row is inserted into the mentions table, this trigger fires.
--   2. If the target is a human user (target_type = 'human'), a notification
--      row is automatically inserted into public.notifications addressed to
--      that user (user_id = target_id).
--   3. Agent targets are skipped — agents receive mention signals through the
--      separate task/webhook system instead.
--   4. The notification includes a deep link to the document + mention so the
--      user can navigate directly to the relevant context.
--
-- Column mapping (mentions → notifications):
--   source_type  → actor_type
--   source_id    → actor_id
--   source_name  → actor_name   (the notifications.actor_name column)
--   context_text → context_snippet
--
-- IMPORTANT: The mentions table does NOT have an actor_name column.
-- We use NEW.source_name from the mentions row for both the notification
-- content string and the actor_name field in the notifications table.
--
-- NOTE: This initial version references is_bot_generated (the original column
-- name). The column is renamed to is_agent_generated in the override section
-- below (Step 2), which re-creates this function with the new column name.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Build notification content using source_name (the mention creator's name).
  -- Note: mentions table has no actor_name column; source_name is the correct field.
  ...
  -- Insert into notifications; the trg_populate_notification_actor BEFORE
  -- INSERT trigger on notifications will auto-populate actor_name if NULL.
  INSERT INTO public.notifications (
    ...
    NEW.source_type,
    NEW.source_id,
    NEW.source_name,        -- maps mentions.source_name → notifications.actor_name
    NEW.context_text,
...
-- ---------------------------------------------------------------------------
-- Trigger: trg_create_notification_on_mention
-- ---------------------------------------------------------------------------
-- Fires AFTER every INSERT on public.mentions.
-- For human targets: inserts a row into public.notifications so the mentioned
-- user sees the mention in their notification stream.
-- For agent targets: no-op (agents use the webhook/task system).
-- ---------------------------------------------------------------------------
...
COMMENT ON TABLE public.mentions IS 'Tracks @mentions in documents. INSERT triggers auto-create notifications for human targets via trg_create_notification_on_mention.';
COMMENT ON COLUMN public.mentions.source_type IS 'human or agent - who created the mention';
COMMENT ON COLUMN public.mentions.target_type IS 'human or agent - who was mentioned. Only human targets receive auto-notifications; agent targets are handled via the task/webhook system.';
...
COMMENT ON COLUMN public.notifications.mention_id IS 'Links to mentions table when type=mention. Auto-populated by trg_create_notification_on_mention.';
COMMENT ON COLUMN public.notifications.actor_name IS 'Display name of the actor. Auto-populated by trg_populate_notification_actor if NULL on insert.';
...
-- Step 2: Recreate the trigger function with the renamed column
-- (is_bot_generated → is_agent_generated). This is the FINAL version that
-- runs in production. See the detailed documentation block above the
-- original CREATE for the full behavior description.
--
-- BUG FIX: The original referenced NEW.actor_name which does not exist on
-- the mentions table. This would cause a runtime error:
--   "record 'new' has no field 'actor_name'"
-- Fixed to use NEW.source_name consistently.
CREATE OR REPLACE FUNCTION public.create_notification_on_mention()
RETURNS TRIGGER AS $$
DECLARE
  v_target_user_id UUID;
