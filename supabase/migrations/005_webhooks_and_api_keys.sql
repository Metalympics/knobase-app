...
CREATE POLICY "Users can manage webhooks for their workspaces"
  ON public.agent_webhooks FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.workspace_members wm
      WHERE wm.workspace_id = agent_webhooks.workspace_id
...
CREATE POLICY "Service role full access to webhooks"
  ON public.agent_webhooks FOR ALL TO service_role
...
CREATE POLICY "Service role full access to API keys"
  ON public.agent_api_keys FOR ALL TO service_role
  USING (true) WITH CHECK (true);
